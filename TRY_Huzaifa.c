...
struct userlogin {
	...
	get_item(char id[8], char str[50]){
	
//			True -- string found
			if(strstr("kit",str)!= NULL){
				total.t_cric.ball++;
				total.t_cric.bat++;
				total.t_cric.wicket++;
				printf("Record Found: id- %s",id);
			}
			else if(strstr("bat",str)!= NULL){
				total.t_cric.bat++;
				printf("Record Found: id- %s\n",id);
				printf("Bat returned successfully\n");
			}
			else if(strstr("wicket",str)!= NULL){
				total.t_cric.wicket++;
				printf("Record Found: id- %s\n",id);
				printf("Wicket returned successfully\n");
			}
			else if(strstr("ball",str)!= NULL){
				total.t_cric.ball++;
				printf("Record Found: id- %s\n",id);
				printf("Ball returned successfully\n");
			}
			
			else if(strstr("volleyball",str)!= NULL){
//			True -- string found
			if(strstr("kit",str)!= NULL){
				total.voll++;
				printf("Record Found: id- %s\n",id);
			...
			else if(strstr("chess", str)!= NULL){
				total.chess++;
				printf("Record Found: id- %s\n",id);
				printf("Chess returned sucessfully");
			}
			else if(strstr("set",str)!=NULL){
				total.t_bad.racket++;
				total.t_bad.shettle++;
